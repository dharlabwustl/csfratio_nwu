...
#    print('sys.argv')
    ...
    lower_thresh=20 #int(float(sys.argv[7])) #20 #0 # 20 #
    upper_thresh=80 #int(float(sys.argv[8])) #80 # 40 # 80 #
...
#                            lineThickness = 2
                    ...
                    cv2.imwrite("/output/I4_img" +grayfilename_base + "_1.jpg",I4)
                    I4_img_1=cv2.imread("/output/I4_img" +grayfilename_base + "_1.jpg")
                    ...
                    if np.sum(I_t_r_f_rinv_tinv_mask)>0 :
                        ...
                        if (infarct_pixels_volume1/1000) > 0.1:
                            ...
                            cv2.imwrite("/output/I4_img" +grayfilename_base + ".jpg",I4)
                            I4_img=cv2.imread("/output/I4_img" +grayfilename_base + ".jpg")
                            ################################################################
                            # font 
...
#                         slice_3_layer_brain = cv2.putText(slice_3_layer,str(slice_number) , org, font,  fontScale, color, thickness, cv2.LINE_AA) 
                        ...
                        if os.path.exists(sys.argv[4])  and int(slice_number) >=int(lower_slice_num) and int(slice_number)<=int(upper_slice_num) :
                            ...
                            latex_insertimage_tableNc(latexfilename,image_list,5, caption="",imagescale=0.18, angle=90,space=0.51)
                            ...
                            # image_list.append(os.path.join(SLICE_OUTPUT_DIRECTORY,image_infarct_details)) 
                            image_list.append(os.path.join(SLICE_OUTPUT_DIRECTORY,imagefilename +".png"))
                            latex_insertimage_tableNc(latexfilename,image_list,2, caption="",imagescale=0.18, angle=90,space=0.51)
